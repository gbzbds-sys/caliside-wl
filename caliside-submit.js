...
function privateChannelName(pseudo, discordId) {
  ...
  return `wl-${base}-${suffix}`.slice(0, 90);
}
...
async function postPrivateApplication(channelId, candidateId, embed, staffLink) {
  const privacyEmbed = {
    ...
    footer: { text: 'CaliSide WL • Salon privé candidat • API 3.9.0' },
  ...
  const sent = await botApi(`/channels/${channelId}/messages`, {
    ...
    body: JSON.stringify({
      ...
      allowed_mentions: { users: [String(candidateId)] },
      embeds: [privacyEmbed]
    })
  });
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      return res.status(200).json({ ok: true, service: 'CaliSide submit', version: '3.9.0' });
    }
    if (req.method !== 'POST') {
      res.setHeader('Allow', 'GET, POST');
      return res.status(405).json({ ok: false, error: 'Méthode non autorisée', version: '3.9.0' });
    }
    ...
    try {
      ...
      return res.status(502).json({ ok:false, error:'Création du salon privé impossible', detail:String(e?.message || e), version:'3.9.0' });
    }
    ...
    const mainEmbed = {
      ...
      description: testMode
        ? '🧪 **Candidature TEST** reçue par l’API V3.9.'
    ...
    const payload = {
      content: `${staffRoles.map(id => `<@&${id}>`).join(' ')}${staffRoles.length ? '\n' : ''}Nouvelle candidature privée de **${pseudo}** • salon <#${privateChannel.id}>`,
      ...
      if (!staffPassword) {
        ...
        if (host) {
          ...
          const staffEmbed = {
            ...
            footer: { text: 'CaliSide WL • Candidature reçue • Panel staff actif • API 3.9.0' }
          };
    ...
    let privatePosted = false;
    let privatePostError = null;
    try {
      await postPrivateApplication(privateChannel.id, did, mainEmbed, staffLink);
      privatePosted = true;
    } catch (e) { privatePostError = String(e?.message || e); }

    return res.status(200).json({ ok: true, via: sent.via, staffLink, staffLinkPosted, staffLinkError, privateChannelId: privateChannel.id, privatePosted, privatePostError, version: '3.9.0' });
  } catch (err) {
    console.error('[CaliSide WL] API 3.9 crash:', err);
