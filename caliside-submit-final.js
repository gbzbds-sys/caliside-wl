...
export default async function handler(req,res){
  res.setHeader('X-CaliSide-Endpoint','caliside-submit-final-v1');
  ...
  const postCandidate=async(payload)=>{
    // Priorité au bot, avec repli automatique sur le webhook s'il est mal configuré.
    if(botToken){
  ...
  const staffPassword=process.env.CALISIDE_STAFF_PASSWORD || process.env.PURPLECITY_STAFF_PASSWORD || 'caliside1616';
  ...
  return res.status(200).json({ok:true,version:'CALISIDE-STABLE-SEND-2026-08-29'});
}
